...
/**
 * A test fixture with helper methods for testing the ROM_EXT implementation of
 * `bootstrap_bounds_check()`.
 */
class RomExtBootstrapBoundsCheck : public ::testing::Test {
 protected:
  OffsetRegion GetRegionRomExtSlotA() const {
    return OffsetRegion(0, CHIP_ROM_EXT_SIZE_MAX);
  }
  OffsetRegion GetRegionRomExtSlotB() const {
    return GetRegionRomExtSlotA().ShiftRight(FLASH_CTRL_PARAM_BYTES_PER_BANK);
  }
  OffsetRegion GetRegionAfterRomExtSlotA() const {
    return OffsetRegion(CHIP_ROM_EXT_SIZE_MAX, FLASH_CTRL_PARAM_BYTES_PER_BANK -
                                                   CHIP_ROM_EXT_SIZE_MAX);
  }
  OffsetRegion GetRegionAfterRomExtSlotB() const {
    return GetRegionAfterRomExtSlotA().ShiftRight(
        FLASH_CTRL_PARAM_BYTES_PER_BANK);
  }
};

// Test that slot A's ROM_EXT is neither erasable nor programmable.
TEST_F(RomExtBootstrapBoundsCheck, RomExtInSlotA) {
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionRomExtSlotA().start()),
            kHardenedBoolFalse);
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionRomExtSlotA().Last()),
            kHardenedBoolFalse);

  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeSectorErase,
                                   GetRegionRomExtSlotA().start()),
            kHardenedBoolFalse);
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeSectorErase,
                                   GetRegionRomExtSlotA().Last()),
  ...
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodePageProgram,
                                   GetRegionRomExtSlotA().start()),
            kHardenedBoolFalse);
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodePageProgram,
                                   GetRegionRomExtSlotA().Last()),
...
// Test that slot A's non-ROM_EXT region is both erasable and programmable.
TEST_F(RomExtBootstrapBoundsCheck, AfterRomExtInSlotA) {
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionAfterRomExtSlotA().start()),
            kHardenedBoolTrue);
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionAfterRomExtSlotA().Last()),
            kHardenedBoolTrue);
...
// Test that slot B's ROM_EXT is neither erasable nor programmable.
TEST_F(RomExtBootstrapBoundsCheck, RomExtInSlotB) {
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionRomExtSlotB().start()),
            kHardenedBoolFalse);
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionRomExtSlotB().Last()),
            kHardenedBoolFalse);
...
// Test that slot B's non-ROM_EXT region is erasable, but not programmable.
TEST_F(RomExtBootstrapBoundsCheck, AfterRomExtInSlotB) {
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionAfterRomExtSlotB().start()),
            kHardenedBoolTrue);
  EXPECT_EQ(bootstrap_bounds_check(kSpiDeviceOpcodeChipErase,
                                   GetRegionAfterRomExtSlotB().Last()),
            kHardenedBoolTrue);
